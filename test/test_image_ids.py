...
@pytest.mark.parametrize("Dockerfile_dir", os.listdir(os.path.join(os.path.dirname(__file__), "test_envs")))
def test_image_ids(Dockerfile_dir):
    test_path = os.path.join(os.path.dirname(__file__), "test_envs", Dockerfile_dir)
    
    dockerfile_name = next(f for f in os.listdir(test_path) if os.path.isfile(os.path.join(test_path, f)) and f.lower().endswith("dockerfile"))
